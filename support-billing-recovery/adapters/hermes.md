...
version: 1.0.0
description: 'Recover a stuck billing charge for a support ticket: identify the charge,
  verify the failure mode, issue the refund or retry, document the resolution, and
  close the loop with the customer. Designed to work identically whether the agent
  runs in Hermes, Claude Code, Cursor, Codex, or a generic shell.
